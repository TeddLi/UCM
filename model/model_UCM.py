...
class UCM(object):
    def __init__(
        ...
        self.dialogue_label =  tf.placeholder(tf.int64, [None], name="dialogue_label")
        ...
        with tf.variable_scope("prediction_layer") as vs:
            ...
            losses = LDAMLoss(logit=logits, labels=flatten_target, m_list=[ 988941, 5996, 27912])
        ...
        with tf.name_scope("Conversation_loss"):
            ...
            self.conv_loss = LDAMLoss(logit=c_logits, labels=self.dialogue_label, m_list = [76763, 28737])
            conv_probs = tf.argmax(tf.nn.softmax(c_logits), 1)

            self.conv_acc = tf.reduce_mean(tf.cast(tf.equal(conv_probs, self.dialogue_label), dtype=tf.float32))
